...
$('signupSelfie').addEventListener('change', async event => { compressedSelfie = null; try { compressedSelfie = await compressImage(event.target.files[0]); } catch (error) { event.target.value = ''; toast(error.message, 'error'); } });
...
$('pendingRows').addEventListener('click', async event => { const approve = event.target.dataset.approve; const reject = event.target.dataset.reject; if (!approve && !reject) return; if (approve && event.target.dataset.hasSymbol !== 'true') return toast('This member did not submit a symbol number. Add it directly in the database before approval.', 'error'); const id = approve || reject; const update = approve ? { status: 'approved' } : { status: 'rejected' }; const { error } = await supabase.from('choir_profiles').update(update).eq('id', id); if (error) return toast(error.message, 'error'); await supabase.rpc('choir_rebuild_aggregate'); await loadAdmin(); toast(approve ? 'Member approved.' : 'Member rejected.'); });
